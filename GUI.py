...
def btseg():
    ...
    if isseg:
        ...
        isseg = True
button_seg = tk.Button(root, text='语义分割开关', width=25, command=btseg)
button_seg.place(x = 1280, y = 560, width=160, height=80)
def btline():
    global isline
    if isline:
        isline = False
        ...
        isline =True
button_line = tk.Button(root, text='车道线检测开关', width=25, command=btline)
button_line.place(x = 1440, y = 560, width=160, height=80)
def btquit():
    root.destroy()
button_quit = tk.Button(root, text='退出', width=25, command=btquit)
button_quit.place(x = 1600, y = 560, width=320, height=80)
